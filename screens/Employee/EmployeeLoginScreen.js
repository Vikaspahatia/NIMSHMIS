...
import { StyleSheet, Text, View, TouchableOpacity, TextInput, Image, Alert, KeyboardAvoidingView, Platform} from 'react-native';
...
const EmployeeLoginScreen = ({navigation}) => {
  ...
  const userAuth = async() =>  {
    ...
    try {
      ...
      const URL1="https://nimsts.edu.in/NIMSNC_MobileServices/service/login/generateotp?user="+empid+"&hcode=33101"; 
      try {
        const content1 = await fetch(URL1)
        const jsonDATA1 = await content1.json()
        OTP = jsonDATA1.OTP;
        console.log(OTP,'otp');
      }  catch (e){
        console.log(e)
