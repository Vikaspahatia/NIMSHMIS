...
const PrescriptionOnlineViewScreen = props => {
  ...
  const patientcrno = props.navigation.getParam('crnoKey');
    ...
    const URL = 'https://nimsts.edu.in/HISServices/service/UserService/getUserPatDataViewPrescription?CrNo='+patientcrno+'&seatid=10001';
    //console.log(URL)
    ...
    useEffect(() => {
      ...
      setDATA(jsonDATA);
      //dataLenght=jsonDATA.pat_details.length
      setdataLenght(jsonDATA.pat_details.length)
      console.log(dataLenght)
      //console.log(DATA)
      setLoading(false);
      // setName(jsonDATA[0].UNITCODE)
      // console.log(jsonDATA[0].UNITCODE);
...
function onPressPass(item) {
  ...
  props.navigation.navigate('POVpdf', {
